...
def plot_variable_distr(covid19_paper_df, flu_paper_df, binning_var):
    for var in binning_control_var:
        fig, axes = plt.subplots(1, 2, figsize=(10, 8))
        sns.distplot(covid19_paper_df[var], ax=axes[0])
        axes[0].set_title("COVID-19 {}".format(var))
        sns.distplot(flu_paper_df[var], ax=axes[1])
        axes[1].set_title("FLU {}".format(var))

        plt.savefig("../plots/{}_distr.png".format(var))
        plt.close()
...
if __name__ == "__main__":
    ...
    # covid19_days_since_pub_converted = covid19_paper_df.days_since_pub.apply(lambda x: 0 if x >= 120 and x <= 150 else (1 if x > 150 and x <= 180 else (2 if x > 180 and x <= 210 else (3 if x > 210 and x <= 240 else (4 if x > 240 and x <= 260 else np.nan)))))
    # covid19_paper_df = covid19_paper_df.assign(days_since_pub_binary = covid19_days_since_pub_converted)
    # flu_days_since_pub_converted = flu_paper_df.days_since_pub.apply(lambda x: 0 if x >= 120 and x <= 150 else (1 if x > 150 and x <= 180 else (2 if x > 180 and x <= 210 else (3 if x > 210 and x <= 240 else (4 if x > 240 and x <= 260 else np.nan)))))
    # flu_paper_df = flu_paper_df.assign(days_since_pub_binary = flu_days_since_pub_converted)
    ...
    covid19_matched = covid19_matched[[predictor_var] + control_var + ["covid19", "citation_per_month_log"]]
    flu_matched = flu_matched[[predictor_var] + control_var + ["covid19", "citation_per_month_log"]]
    ...
    citation_count_tf_model = cem_test("citation_per_month_log", "topic_familiarity", combined_papers_scaled, [predictor_var] + ["new_tie_rate"] + ["topic_distr{}".format(i) for i in range(1,20)])
    # citation_count_ntr_model = cem_test("citation_per_month_log", "new_tie_rate", combined_papers_scaled, [predictor_var] + control_var)
