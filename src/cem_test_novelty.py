...
def cem_test(target_colname, var, combined_papers_scaled, feature_var):
    ...
    # formula = "{} ~ covid19:{} + covid19 + {}".format(target_colname, var, "+".join(aftervif.columns))
...
if __name__ == "__main__":
    ...
    covid19_export_variables = covid19_matched[['topic_familiarity', 'covid19','citation_count_log',\
            'citation_counts_binary_20perc','citation_counts_binary_10perc','citation_counts_binary_5perc','citation_counts_binary_1perc',\
            'novelty_10perc','novelty_5perc','novelty_1perc']]
    flu_export_variables = flu_matched[['topic_familiarity', 'covid19','citation_count_log',\
            'citation_counts_binary_20perc','citation_counts_binary_10perc','citation_counts_binary_5perc','citation_counts_binary_1perc',\
            'novelty_10perc','novelty_5perc','novelty_1perc']]
    combined_papers_export = pd.concat([covid19_export_variables, flu_export_variables])
    combined_papers_export_scaled = combined_papers_export.copy()
    X = np.array(combined_papers_export_scaled[['topic_familiarity']])
    X_scaled = scale(X)
    combined_papers_export_scaled[['topic_familiarity']] = X_scaled
    combined_papers_export_scaled.to_csv("../results/cem_test_interaction_term_data_exported.csv", index=False)

    # Combine covid and flu papers
    covid19_matched = covid19_matched[[predictor_var] + control_var + ["covid19", "novelty_10perc", "novelty_5perc", "novelty_1perc"]]
    flu_matched = flu_matched[[predictor_var] + control_var + ["covid19", "novelty_10perc", "novelty_5perc", "novelty_1perc"]]
    ...
    novelty10_tf_model = cem_test("novelty_10perc", "topic_familiarity", combined_papers_scaled, [predictor_var] + control_var)
    novelty5_tf_model = cem_test("novelty_5perc", "topic_familiarity", combined_papers_scaled, [predictor_var] + control_var)
    novelty1_tf_model = cem_test("novelty_1perc", "topic_familiarity", combined_papers_scaled, [predictor_var] + control_var)

    novelty10_ntr_model = cem_test("novelty_10perc", "new_tie_rate", combined_papers_scaled, [predictor_var] + control_var)
